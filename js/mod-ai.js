...
(function () {
  ...
  async function jyfsCore(o) {
    ...
    const ctxOverride = o.ctxText || "";
    let modKey = (o.modKey !== undefined) ? o.modKey : null;
    if (modKey === null) {
      const K = window.KGAIQuiz;
      modKey = (qCtx && qCtx.subject && K) ? K.moduleForSubject(qCtx.subject)
        : (K ? (K.moduleForSubject(curLog.map(m => m.content || "").join("\n")) || null) : null);
    }
    if (modKey === "essay") throw new Error("申论暂不支持出题");
    const isQuestion = !!(qCtx && qCtx.q) && !ctxOverride;
    const hasOrigImg = !!(isQuestion && qCtx.q && qCtx.q.img);
    const boardKey = (modKey && modKey !== "ai") ? modKey : "ai";
    const modTitle = (modKey && modKey !== "ai") ? ((window.MODULES[modKey] || {}).title || modKey) : "综合";
    ...
    // 模型单次输出上限（如智谱 GLM-4V-Flash 只有 1024）：决定单次能出几题、是否精简输出
    ...
    for (let r = 0; r < rounds; r++) {
      ...
      if (cnt <= 0) break;
      if (prog && rounds > 1) prog(`⏳ AI 正在出题…（第 ${r + 1}/${rounds} 批，共 ${n} 题）`);
  ...
  function jyfsFinish(res) {
    ...
    if (res.boardKey === curKey) {
      // 已在该模块页：hash 不变不会触发重渲染，手动刷新以挂载板块并自动开训
      if (typeof window.renderRoute === "function") window.renderRoute();
    } else {
      location.hash = "#/" + res.boardKey;
    }
    return set;
  }

  /* 自动生成一组新题并直接进入训练（供「下一组 / 再来一组」调用） */
  async function jyfsAuto(n, o) {
    o = o || {};
    UI.toast("⏳ AI 正在出新题，请稍候…");
    try {
      const res = await jyfsCore(Object.assign({}, o, { n: n }));
      return jyfsFinish(res);
    } catch (e) {
