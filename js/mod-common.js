    ...
    render(body) {
      ...
      try {
        ...
        if (KJ && KJ.chapters && KJ.chapters.length) {
          ...
          /* ===== 进入常识口诀板块：主动询问是否从上次浏览处继续 ===== */
          try {
            const kv = (DB.state.common && DB.state.common.kjView) || null;
            const freshKv = kv && kv.ts && (Date.now() - kv.ts < 7 * 864e5);
            if (!freshKv) { /* 7 天内无浏览记录，不弹 */ }
            else {
              const isLearn = (kv.type === "learn" || kv.type === "review");
              const isBrowse = (kv.type === "browse");
              // 同一会话内、同一位置只提示一次；位置变了再提示；杀后台重开（新会话）会再提示
              const sig = "kjResume_" + (isLearn ? ("L" + (kv.start || 0)) : ("B" + (kv.chapter)));
              let shown = false;
              try { shown = sessionStorage.getItem(sig) === "1"; } catch (e) {}
              if (!shown) {
                try { sessionStorage.setItem(sig, "1"); } catch (e) {}
                const chName = (isBrowse && KJ.chapters[kv.chapter]) ? KJ.chapters[kv.chapter].name : "";
                const desc = isLearn ? `上次浏览到「第 ${(kv.start || 0) + 1} 条」`
                  : (isBrowse ? `上次通览到「${chName}」` : "");
                const acts = [];
                if (isLearn) acts.push({
                  label: "📖 回到上次浏览处（第 " + ((kv.start || 0) + 1) + " 条）", cls: "primary",
                  onClick: (m, c) => { c(); if (kv.type === "review") { learnPage(0, "review", dueList()); } else { learnPage(kv.start || 0, "learn"); } }
                });
                if (isBrowse) acts.push({
                  label: "📑 回到上次通览（" + (chName.length > 10 ? chName.slice(0, 10) + "…" : chName) + "）", cls: "primary",
                  onClick: (m, c) => { c(); openBrowse(kv.chapter); }
                });
                acts.push({ label: "不用了", cls: "ghost", onClick: (m, c) => c() });
                UI.modal({
                  title: "🧿 常识口诀88条 · 继续上次浏览？",
                  body: `<p class="muted small" style="line-height:1.8">检测到你 ${desc}，是否跳转到上次的位置继续？</p>`,
                  width: "460px",
                  actions: acts
                });
              }
