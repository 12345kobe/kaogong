...
OUT_HISTORY = os.path.join(ROOT, "assets", "data", "hotspot_history.js")
OUT_HISTORY_SITE = os.path.join(ROOT, "site", "assets", "data", "hotspot_history.js")
...
# 历史累计：把每次抓到的热点长期保留，供「历史时政」归档（按月/日回溯）
HIST_MAX = 2000      # 历史文件最多保留多少条（超出按日期保留最新）
HIST_DAYS = 730      # 历史只保留最近 N 天（约两年）
...
def load_history():
    ...
    try:
        with open(OUT_HISTORY, encoding="utf-8") as f:
            txt = f.read()
        i = txt.find("=")
        j = json.loads(txt[i + 1:].rstrip().rstrip(";"))
        return j if isinstance(j, dict) else {"items": []}
    except Exception:
        return {"items": []}
...
def save_history(items):
    """把本次抓到的热点合并进历史文件：按「标题+日期」去重，保留最近 HIST_DAYS 天、最多 HIST_MAX 条。"""
    hist = load_history()
    existing = hist.get("items", []) if isinstance(hist, dict) else []
    by_key = {}
    for e in existing:
    ...
    merged = [m for m in merged if 0 <= _age_days(m.get("date", "")) <= HIST_DAYS]
    merged.sort(key=lambda x: x.get("date", "") or "0000", reverse=True)
    merged = merged[:HIST_MAX]
    data = {
        "updatedAt": datetime.datetime.now().strftime("%Y-%m-%d %H:%M"),
        "count": len(merged),
        "items": merged,
    }
    js = "window.KG_HOTSPOTS_HISTORY = " + json.dumps(data, ensure_ascii=False, indent=1) + ";\n"
    for o in (OUT_HISTORY, OUT_HISTORY_SITE):
        try:
